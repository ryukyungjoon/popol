...
import numpy as np
import pandas as pd
from keras.layers import Lambda, Dense, Conv1D, LeakyReLU, Flatten, MaxPooling1D
from keras.optimizers import Adam
from keras.utils import to_categorical
from numba.tests.serialize_usecases import K
...
class CNNSiameseNetwork:
    def __init__(self, train_x, train_y, test_x, test_y, n_epochs=100, batch_size=16, latent_dim=32):
        ...
        ''' Train & Test data class 별로 분리 (train_groups[0] : normal, train_groups[1] : DoS ...)'''
        self.train_groups = [train_x[np.where(train_y == i)[0]] for i in np.unique(train_y)]
        self.test_groups = [test_x[np.where(test_y == i)[0]] for i in np.unique(test_y)]
    ...
    ''' Training data get random batch '''
    def gen_random_batch(self, in_groups, batch_halfsize=1):
        """

        :param in_groups: train_groups or test_groups, list
        :param batch_halfsize: match group(1)(halfsize), not match group(0)(halfsize)
        :return: match_group, not match_group, match_score(0 or 1), stack
        """
    ...
    def siam_gen(self, in_groups, batch_size=5):
        while True:
            pv_a, pv_b, pv_sim = self.gen_random_batch(in_groups, batch_size // 2)
            yield [pv_a, pv_b], pv_sim

    def run(self):
        # 5-shot, make up validation set
