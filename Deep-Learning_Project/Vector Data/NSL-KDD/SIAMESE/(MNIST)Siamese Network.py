...
class CNNSiameseNetwork:
    def __init__(self, train_x, train_y, test_x, test_y, n_epochs=100, batch_size=16, latent_dim=32):
        ...
        # self.cls_model = self.cls_conv(self.left_input)
        self.siam_cls = Model(self.left_input, self.build_classifier(self.convnet(self.left_input)))

    # def cls_conv(self, input_data):
    #     conv_filter = [128, 64, 32]
    #     model = Sequential()
    #
    #     model.add(Conv1D(conv_filter[0], 3, input_shape=self.input_dim))
    #     model.add(LeakyReLU(alpha=0.2))
    #
    #     model.add(Conv1D(conv_filter[1], 3))
    #     model.add(LeakyReLU(alpha=0.2))
    #
    #     model.add(Conv1D(conv_filter[2], 3))
    #     model.add(LeakyReLU(alpha=0.2))
    #
    #     model.add(Flatten())
    ...
    def build_classifier(self, input_data):
        cls = Dense(122, activation='relu')(input_data)
    ...
    def main(self):
        valid_a, valid_b, valid_sim = self.gen_random_batch(self.train_groups, 25)

        history = self.siamese_net.fit_generator(self.siam_gen(self.train_groups, 100),
        ...
        plt.title('Siamese Model Training and Validation loss')
        plt.legend()
        plt.show()

        self.siamese_net.save_weights('./aswd.h5')   # save weight

        ## Load weights and Set weights
        # self.siamese_net.load_weights('./aswd.h5')
        for i, layer in enumerate(self.siam_cls.layers[0:10]):
            print(i, layer)
            layer.set_weights(self.siamese_net.layers[i*2].get_weights())
            layer.trainable = False

        self.siam_cls.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
