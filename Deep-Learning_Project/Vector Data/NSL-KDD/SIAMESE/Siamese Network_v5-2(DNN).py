...
class CNNSiameseNetwork:
    def __init__(self, train_x, train_y, test_x, test_y, n_epochs=100, batch_size=16, latent_dim=32):
        ...
        left_feat = self.densenet(left_input)
        right_feat = self.densenet(right_input)
        ...
        # merge_layer = concatenate([self.compare.output[0], self.compare.output[1]])
        ...
        self.siam_cls = Model(left_input, self.build_classifier(self.densenet(left_input)), name='siam_cls')
    ...
    def densenet(self, input_data):
        Nh_l = [512, 256, 128]
        den = Dense(Nh_l[0], activation='relu', input_shape=(122,))(input_data)
        den = Dense(Nh_l[1], activation='relu')(den)
        den = Dense(Nh_l[2], activation='relu')(den)
        # den = Dense(Nh_l[3], activation='relu')(den)
        # den = Dense(Nh_l[4], activation='relu')(den)
        # den = Lambda(lambda x: K.l2_normalize(x, axis=-1))(den)
    ...
    def main(self):
        ...
        with open('./model_save/siamese_net(DNN).json', 'w') as json_file:
            json_file.write(model_json)
        self.siamese_net.save_weights('./model_save/siamese_net(DNN).h5')

        ## Classifier Training ##
        print(self.onehot_train_y)
        history = self.siam_cls.fit(self.train_x, self.onehot_train_y,
                                    batch_size=32, epochs=64, verbose=1,
        ...
        for layer in self.siam_cls.layers[0:4]:
...
if __name__ == '__main__':
    ...
    print("=========== DNN Siameses Network init ============")
